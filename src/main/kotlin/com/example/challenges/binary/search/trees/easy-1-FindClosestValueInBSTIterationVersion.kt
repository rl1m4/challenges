...
fun main() {
    ...
    println(findClosestValueInBstIteration(tree, target))
}

fun findClosestValueInBstIteration(tree: BST, target: Int): Int =
    findIt(tree, target, tree.value)

fun findIt(tree: BST, target: Int, closest: Int): Int {
    var newClosest = closest
    var currentNode = tree

    while (true) {
        if (abs(target - newClosest) > abs(target - currentNode.value)) currentNode.value.also { newClosest = it }

        currentNode = if (target < currentNode.value && currentNode.left != null) {
            currentNode.left!!
